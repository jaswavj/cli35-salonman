...
const empty = { fullName: '', userName: '', password: '', shopId: '' };
...
const CreateUserPage: React.FC = () => {
  ...
  useEffect(() => {
    ...
    usersApi.outlets()
      .then((res) => setOutlets(usersData<Outlet[]>(res) || []))
      .catch((err) => toast.error(usersError(err, 'Could not load outlets')));
  }, []);
  ...
  const onSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!form.fullName.trim() || !form.userName.trim() || !form.password.trim() || !form.shopId) {
      toast.warning('Full name, username, password and outlet are required');
      return;
    }
    setBusy(true);
    try {
      ...
      <form className="usr-layout" onSubmit={onSubmit}>
        <div className="mst-card usr-form-card">
          ...
          <div className="mst-card-b">
            ...
            <div className="mst-fg">
              <label>Outlet <span className="req">*</span></label>
              <select
                className="mst-sel"
                value={form.shopId}
              ...
              >
                ...
                {outlets.map((o) => (
                  <option key={o.shopId} value={o.shopId}>{o.shopName}</option>
                ))}
              </select>
            </div>
