...
const Login = () => {
  ...
  const [user, setUser] = useState({ shopId: '', userName: '', password: '' })
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      dispatch(authLogout())
      const response = await loginApiService.loginUser({
        shopId: user.shopId.trim(),
      ...
      } else if (!apiError || apiError === 'Invalid Credentials' || error?.response?.status === 401) {
        setErrorMessage('Invalid shop ID, username or password')
      } else {
    ...
    <div className="login-page">
      <div className="login-board">
        ...
        <section className="login-form-side">
          ...
          <p className="login-form-sub">Use your shop ID, username and password.</p>
          ...
          <form onSubmit={handleLogin}>
            <div className="login-field">
              <label htmlFor="login-shop-id">Shop ID</label>
              <div className="login-input-wrap">
                <FontAwesomeIcon icon={faStore} className="login-input-icon" />
                <input
                  id="login-shop-id"
